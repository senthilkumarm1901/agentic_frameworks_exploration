"""Hermes-style ReAct agent with MCP tools for country-data analysis.
...
Key Hermes library practices reflected here:
- Constructor params: model, quiet_mode, ephemeral_system_prompt, max_iterations,
  skip_memory, skip_context_files, base_url, api_key
- run_conversation() returns {final_response, messages, ...}
- chat() is a convenience wrapper over run_conversation()
- task_id param on run_conversation() for isolation (no-op locally)
"""
...
_MCP_SERVER_PATH = _PROJECT_ROOT / "_shared" / "src" / "mcp_servers" / "pattern1_server.py"


class HermesAgent:
    """Hermes-pattern ReAct agent using hermes3 via Ollama + MCP tools.
    ...
    def run_conversation(
        ...
        Mirrors ``AIAgent.run_conversation()`` return structure so callers can
        access ``result['final_response']`` and ``result['messages']`` as
        documented in the hermes-agent library.

        Additional keys (not in the library interface) carry per-run metrics:
        llm_calls, tool_calls, tool_calls_detail, prompt_tokens,
        completion_tokens, total_tokens, total_duration_ms.
        """
        import asyncio
    ...
    async def _run_async(
        ...
        async with stdio_client(
            StdioServerParameters(command="python", args=[str(_MCP_SERVER_PATH)])
        ) as (read, write):
