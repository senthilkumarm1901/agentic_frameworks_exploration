...
- MCP tools (country_lookup, calculator, wiki_read — NO RAG)
...
- Replaces vector RAG (kb_search) with LLM Wiki (wiki_read)
- No sentence-transformers or Milvus dependencies
"""
...
for logger_name in ["httpx", "httpcore", "grpc", "mcp", "mcp.server"]:
...
_MCP_SERVER_PATH = str(_PROJECT_ROOT / "_shared" / "src" / "mcp_servers" / "pattern5_server.py")
...
def _get_mcp_config() -> dict:
    """Get MCP server configuration for Pattern 5 (wiki-based)."""
    return {
        "country_tools": {
            "command": "python",
...
async def run_chat_turn(
    ...
    Returns:
        Dict with full metrics matching Pattern 3/4 format
    """
    ...
    # Extract metrics + answer (matching Pattern 3/4 format)
    tool_calls_detail = []
    tools_used = []
    llm_calls = 0
    skill_activations = 0
    ...
    return {
        "answer": answer,
        "framework": "langgraph",
        "pattern": "agent_with_memory_no_rag",
