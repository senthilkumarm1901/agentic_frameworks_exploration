...
- Uses wiki_index_tool to discover available pages
"""
...
- wiki_read_tool: Read wiki pages about countries (entities, concepts, comparisons)
- wiki_index_tool: List all available wiki pages and topics

## WIKI USAGE
The wiki contains pre-compiled knowledge organized as:
- Entity pages: Individual country profiles (e.g., "japan", "india")
- Concept pages: Cross-cutting topics ("demographics", "trade_and_economy", "geography")
- Comparisons: Multi-country rankings and analysis

To use the wiki:
1. Call wiki_index_tool() to see available pages
2. Call wiki_read_tool("page_name") for exact pages
3. Call wiki_read_tool("keyword") to fuzzy-match topics
...
- Never invent facts — always use wiki_read_tool
