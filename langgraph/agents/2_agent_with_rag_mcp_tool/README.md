...
Pattern 2 extends Pattern 1 by adding a **RAG (Retrieval-Augmented Generation)** tool that searches a vector database of country facts. This allows the agent to answer questions about culture, history, geography, and other qualitative aspects that aren't in the structured JSON data.
...
```bash
cd langgraph/agents/2_agent_with_rag_mcp_tool
uv sync
