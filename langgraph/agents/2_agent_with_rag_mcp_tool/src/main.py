"""Agent with RAG MCP tool — CLI entry point.

Runs a LangGraph ReAct agent connected to an MCP server for country data queries,
including RAG-based knowledge base search for qualitative facts.

Pattern 2 extends Pattern 1 by adding a country_kb_search_tool for answering
hybrid questions that require both numeric data and qualitative information.

Usage:
    uv run python -m src.main --task country_analysis --question "Brazil has the Amazon rainforest — what is its population density?"
"""
...
# Default question if none provided — hybrid question demonstrating RAG capability
...
def main():
    parser = argparse.ArgumentParser(
        description="Agent with RAG MCP tool (LangGraph + MCP) — Country data Q&A with knowledge base search"
    )
    parser.add_argument(
        "--task",
        required=True,
    ...
    parser.add_argument(
        ...
        help="Natural language question about country data (GDP, population, area, or qualitative facts)",
    )
