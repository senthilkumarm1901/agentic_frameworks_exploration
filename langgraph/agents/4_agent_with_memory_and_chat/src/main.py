"""Interactive terminal chat with memory — Pattern 4 entry point.
...
def _ensure_log_header():
    ...
    if not LOGS_FILE.exists() or LOGS_FILE.stat().st_size == 0:
        header = f"""# Pattern 4: Agent with Memory and Chat Capability - Chat Logs
...
def log_turn_metrics(result: dict, user_query: str, turn_count: int):
    ...
    # Create formatted entry like Pattern 3
...
async def chat_loop():
    ...
    while True:
        ...
        try:
            ...
            # Log metrics to logs.txt (same format as Pattern 3)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Country Analysis Agent — Interactive Chat with Memory"
    )
    parser.add_argument(
        "--reset", 
        action="store_true", 
