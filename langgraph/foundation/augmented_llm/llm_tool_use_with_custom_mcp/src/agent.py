"""LangGraph ReAct agent with MCP client for country data tools.

This module creates a single ReAct agent that connects to the MCP server
via langchain-mcp-adapters, allowing the LLM to call country_lookup and
calculator tools through the MCP protocol.
"""
...
# Resolve the path to the MCP server relative to the workspace root
# Path: src/agent.py -> llm_tool_use_with_custom_mcp -> augmented_llm -> foundation -> langgraph -> workspace_root
