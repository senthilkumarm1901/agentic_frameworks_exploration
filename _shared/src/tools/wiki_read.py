...
def _load_index() -> dict[str, list[str]]:
    ...
    for match in re.finditer(pattern, index_content):
        ...
        page_path_l = page_path.strip().lower()
        page_path_no_ext = page_path_l[:-3] if page_path_l.endswith('.md') else page_path_l
        
        # Map page name and all keywords to the path
        for kw in [page_name.lower(), page_path_l, page_path_no_ext] + keywords:
            if kw:
...
def wiki_read(page_or_query: str) -> str:
    ...
    # Strategy 0: Direct path into llm_wiki, allowing optional .md extension
    query_path = query if query.endswith('.md') else f"{query}.md"
    direct_wiki_path = (_WIKI_ROOT / query_path).resolve()
    if direct_wiki_path.exists() and direct_wiki_path.is_file() and _WIKI_ROOT.resolve() in direct_wiki_path.parents:
        return direct_wiki_path.read_text(encoding="utf-8")
    ...
    # Strategy 3: Partial keyword match (disabled for slash queries to avoid broad collisions)
    if '/' not in query:
    ...
    fuzzy_candidates = list(keyword_map.keys())
    if '/' in query:
        path_like = [k for k in fuzzy_candidates if '/' in k]
        if path_like:
            fuzzy_candidates = path_like

    best_match = _fuzzy_match(query, fuzzy_candidates)
    if best_match:
