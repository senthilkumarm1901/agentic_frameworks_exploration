...
Strands adaptation:
- Uses direct LLM invocation via OllamaModel for summarization (no LangChain)
- Session state (turn_count) is managed manually in main.py
"""
...
def build_summarization_prompt(exchanges: list[tuple[str, str]], max_chars: int = MAX_SUMMARY_CHARS) -> str:
    ...
    Args:
        exchanges: List of (user_query, agent_answer) pairs from this session
    ...
    """
    conversation_lines = []
    for user_q, agent_a in exchanges[-20:]:  # Keep last 20 exchanges max
        conversation_lines.append(f"User: {user_q}")
        truncated = agent_a[:300] + "..." if len(agent_a) > 300 else agent_a
