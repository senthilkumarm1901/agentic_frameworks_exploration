"""Routing pattern — classify input and route to specialized handler (Strands)."""
import argparse
import sys
import time
...
def main():
    parser = argparse.ArgumentParser(description="Routing pattern (Strands)")
    ...
    t0 = time.perf_counter()
    handle_resp = agent(HANDLER_PROMPTS[category].format(message=message))
    metrics.record_llm_call(duration_ms=(time.perf_counter() - t0) * 1000)
